...
Los clientes deben ser ordenados por código de provincia ascendente. */SELECT c.clie_codigo, c.clie_razon_social, count(*),	(SELECT ISNULL(SUM(i1.item_cantidad), 0) FROM Item_Factura i1	JOIN Factura f1 ON (f1.fact_tipo+f1.fact_sucursal+f1.fact_numero = i1.item_tipo+i1.item_sucursal+i1.item_numero)	WHERE YEAR(f1.fact_fecha) = 2012 AND f1.fact_cliente = c.clie_codigo) cant_vendido_2012,	(SELECT TOP 1 i2.item_producto FROM Item_Factura i2	JOIN Factura f2 ON (f2.fact_tipo+f2.fact_sucursal+f2.fact_numero = i2.item_tipo+i2.item_sucursal+i2.item_numero)	WHERE YEAR(f2.fact_fecha) = 2012 AND f2.fact_cliente = c.clie_codigo	GROUP BY i2.item_producto	ORDER BY ISNULL(SUM(i2.item_cantidad), 0) DESC, i2.item_producto ASC) prod_mas_vendido_2012FROM Cliente cJOIN Factura f ON (f.fact_cliente = c.clie_codigo)GROUP BY c.clie_codigo, c.clie_razon_socialHAVING COUNT(f.fact_tipo+f.fact_sucursal+f.fact_numero) <	(SELECT TOP 1 AVG(i1.item_cantidad) FROM Item_Factura i1	JOIN Factura f1 ON (f1.fact_tipo+f1.fact_sucursal+f1.fact_numero = i1.item_tipo+i1.item_sucursal+i1.item_numero)	WHERE YEAR(f1.fact_fecha) = 2012	GROUP BY i1.item_producto	ORDER BY 1 DESC) / 3